...
1. **Survey existing design context.** `Docs/Docs/Moon/ROADMAP.md`, the relevant `Docs/Docs/Moon/Roadmaps/*.md`, and `Docs/Adr/` — don't start from a blank slate if a decision already exists.
...
5. **Update `Docs/Docs/Moon/ROADMAP.md`** with the plan if the work spans more than one session, so it survives context loss.
