...
1. **Roadmap → changelog.** When an item completes, delete/move it out of `Docs/Docs/Moon/ROADMAP.md` and add a dated entry to `Docs/CHANGELOG.md` citing concrete artifacts changed.
...
5. **Consistency check.** Before finishing any documentation PR, grep for the changed fact across `Docs/Moon/`, `Docs/`, and `.agent/` to make sure no stale copy remains.
