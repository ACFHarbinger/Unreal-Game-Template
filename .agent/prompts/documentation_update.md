...
**Intent:** Keep `.agent/`, `Docs/`, and `Moon/` documentation synchronized with a completed code change.
...
Task:
1. If this completes a `Moon/ROADMAP.md` item: move it to `Docs/CHANGELOG.md` in the established dated-entry format, citing concrete files/functions/config keys touched — not a vague summary.
2. If this changes system design: update `Docs/ARCHITECTURE.md` and the relevant `Moon/Roadmaps/*.md` file in the same change. If the decision is significant and hard to reverse, add a new numbered ADR under `Docs/Adr/` instead of only updating `ARCHITECTURE.md`.
