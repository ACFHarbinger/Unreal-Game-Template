...
3. Check `Docs/Docs/Moon/Roadmaps/architecture.md` and the relevant domain roadmap for prior option analysis on this exact area.
...
5. Recommend an approach, with trade-offs, in the same format as the existing `Docs/Docs/Moon/Roadmaps/*.md` files (Options A/B/C, pros/cons, recommendation) so it can be appended directly if approved.
